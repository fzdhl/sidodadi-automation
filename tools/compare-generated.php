...
$paths = [
    __DIR__ . '/debug-usaha.docx',
    dirname(__DIR__) . '/resources/templates/HASIL-RETRY-usaha.docx',
];

foreach ($paths as $path) {
    try {
        $generator->generate('usaha', $values, $path);
        echo "Generated {$path} ok. Size: " . filesize($path) . "\n";
    } catch (Throwable $e) {
        echo "Failed to generate {$path}: " . get_class($e) . ': ' . $e->getMessage() . "\n";
    }
}

foreach ($paths as $path) {
    if (file_exists($path)) {
        echo "MD5 {$path}: " . md5_file($path) . "\n";
    }
}
